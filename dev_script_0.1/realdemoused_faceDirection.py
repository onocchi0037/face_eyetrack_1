...
def main(Persons,
    ...
    predictor68 = dlib.shape_predictor(predictor_path68)
    ...
    with tf.Session() as sess:
        ...
        cap.set(cv2.CAP_PROP_BRIGHTNESS, 60) #明るさ補正 デフォルト128
        cap.set(cv2.CAP_PROP_SATURATION, 135) #彩度補正 デフォルト128
        cap.set(cv2.CAP_PROP_CONTRAST, 90) #コントラスト補正 デフォルト128
        cap.set(cv2.CAP_PROP_GAIN, 120) #ゲイン補正 デフォルト128
        ...
        print("%s, %f" %('CAP_PROP_AUTO_EXPOSURE',cap.get(cv2.CAP_PROP_AUTO_EXPOSURE)))
        ...
        while True:
            ...
            if dummy:
                image = cv2.imread("group-shot.jpg")
            if ret_val != True:
                continue
            ...
            for human in result_keypoints:
                ...
                if nose_x and leftEye_x and rightEye_x:
                    ...
                    if face_bbox_halflen:
                        ...
                        nose_angle = 3.14*35/180
                        nose_pitch_offsets = -0.1
                        if  eye_nose_dist_2d < eye_nose_dist_3d:
                        ...
                        dilb_bbox_size = 0
                        dist = None
                        if dets:    #顔を認識できたら認識できた顔の数 以下を実行
                            d = dets[0]
                            ...
                            # 視線を検知
                            ret, lOSmap = lineOfSight.lOS_main(detector, predictor68, face_image, 0, 500, 1.0, 1.0)
                        ...
                        """
                        if nose_yaw:
                            Ydist = np.array([55,60,70,80,90,100,110,120,130,140,150,160,170,180,190,200,210,220,230,240,250,260,270,280,290,300,320,340,360,380,400])
                            face_dist = eye_between_pn_dist
                            Xface_dist = np.array([198.13,183.69,153.60,132.58,116.85,98.95,91.05,83.16,77.29,69.67,64.29,60.34,56.76,53.17,49.59,47.49,45.38,43.28,41.17,39.07,37.78,36.49,35.20,33.91,32.62,31.34,29.55,27.76,25.98,24.19,22.40])
                            if ear_between_pn_dist:
                                face_dist = ear_between_pn_dist
                                Xface_dist = np.array([449.12,419.77,357.13,301.77,250.70,226.98,205.32,188.12,173.92,162.97,151.35,139.67,132.30,123.90,119.85,114.25,110.11,105.96,101.81,98.19,94.57,90.95,87.33,83.71,80.09,76.47,72.50,68.53,64.56,60.60,56.63])
                        ...
                        #ret, lOSmap = lineOfSight.lOS_main(detector, predictor, face_image, 0, 500, 1.0, 1.0)
                        #print(lOSmap)
                        count = 0
            ...
            if len(np.array(FaceAPIimage[0]).shape) == 3:
                cv2.imwrite("test00.png",FaceAPIimage[0])
            ...
            cv2.imwrite('faces.png', face_images)
            #app_display_image = cv2.resize(display_image,(display_image_width//2,display_image_height//2))
            cv2.imwrite('display_image.png', display_image)
            if view:
