...
def main(Persons,
    ...
    with tf.Session() as sess:
        ...
        eye_yaw =-1
        model_w = keras.models.load_model('./model_w.h5', compile=False)
        model_h = keras.models.load_model('./model_h.h5', compile=False)
        ...
        while True:
            ...
            for human in result_keypoints:
                ...
                if nose_x and leftEye_x and rightEye_x:
                    ...
                    if leftEye_x:
                        face_bbox_halflen_l = 2.6 * abs(leftEye_x - nose_x)
                    if rightEye_x:
                        face_bbox_halflen_r = 2.6 * abs(nose_x - rightEye_x)
                    ...
                    if face_bbox_halflen:
                        ...
                        if len(faceAPI_data) != 0 and type(faceAPI_data) is dict and 'err' not in faceAPI_data:    #顔を認識できたら認識できた顔の数 以下を実行
                            ...
                            #predict_w_rad = 0
                            #for i in range(1, len(r_w)):
                            #    predict_w_rad += r_w[i]*10*i
                            #predict_w_rad = -np.deg2rad(predict_w_rad - 40)
                            #nose_yaw = predict_w_rad
                            ...
                            nose_yaw = -np.deg2rad(r_w_deg)
                            nose_pitch = -np.deg2rad(r_h_deg)
            ...
            for face_id in face_data_dict:
                ...
                if face_id in faceAPIdata_dict:
                    if faceAPIdata_dict[face_id]:
                        ...
                        Status["gender"] = faceAPIdata_dict[face_id]["gender"]
                        Status["age"] = faceAPIdata_dict[face_id]["age"]
...
if __name__ == "__main__":
    ...
    parser.add_argument('--scale_factor', type=float, default=0.36)
    ...
    parser.add_argument('--detect_dist_max', type=float, default=4, help="unit m. default 5")
    parser.add_argument('-n', type=int, default=5, help="Average number of eyetracking data. default 3")
