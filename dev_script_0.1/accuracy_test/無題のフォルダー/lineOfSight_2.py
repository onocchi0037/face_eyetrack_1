...
def lOS_main(parts, inputImage, outputImage, m, rxk, lxk):
    ...
    if dets:
        ...
        ret, face_yaw, face_roll = pupil_faceangle(parts)

        if abs(face_roll) > 2.5 or ret is False:
            #print("roll error")
            return img, lOSmap
        ...
        #print("")
        #img2 = draw_headCenter(img, parts)
        #img2 = draw_monitor(img2)
        img, lOSmap = lineOfSight(img, centerxper_L, centerxper_R, face_yaw, face_roll, eye_scale, rxk, lxk)
...
def pupil_faceangle(parts):
    try:
        ...
        return True, face_yaw, face_roll
    except:
...
def pupil_eyeArea(parts, left=True):
    ...
    if left:
        if parts[38][0] > parts[37][0]:
            minParts = parts[37]
        else:
            minParts = parts[38]
        if parts[41][0] > parts[40][0]:
            maxParts = parts[41]
        else:
            maxParts = parts[40]
        ...
        if parts[44][0] > parts[43][0]:
            minParts = parts[43]
        else:
            minParts = parts[44]
        if parts[46][0] > parts[47][0]:
            maxParts = parts[46]
        else:
            maxParts = parts[47]
    ...
    eyeRelpoint = []
    eyeRelpoint.append([eyepoint6[0][0]-org_x + 1, eyepoint6[0][1]-org_y])
    eyeRelpoint.append([eyepoint6[1][0]-org_x, eyepoint6[1][1]-org_y + 1])
    eyeRelpoint.append([eyepoint6[2][0]-org_x, eyepoint6[2][1]-org_y + 1])
    eyeRelpoint.append([eyepoint6[3][0]-org_x - 1, eyepoint6[3][1]-org_y])
    eyeRelpoint.append([eyepoint6[4][0]-org_x, eyepoint6[4][1]-org_y - 1])
    eyeRelpoint.append([eyepoint6[5][0]-org_x, eyepoint6[5][1]-org_y - 1])

    if is_close(org_y, eyes[2][1]):
        return None, None

    eyearea = [org_y, eyes[2][1], org_x, eyes[-1][0]]
