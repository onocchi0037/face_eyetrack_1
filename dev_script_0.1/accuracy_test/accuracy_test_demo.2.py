...
def main(Persons,
    ...
    with tf.Session() as sess:
        ...
        test_num_list       = [ 0, 0, 3, 3, 1, 1, 2, 2,-1] # テストの順場
        test_time_list      = [ 1, 1, 1, 1, 1, 1, 1, 1, 1]
        test_interval_list  = [ 1, 1, 1, 1, 1, 1, 1, 1, 1]
        ...
        while True:
            ...
            for face_id in face_data_dict:
                ...
                if eye_w and eye_h:
                    ...
                    # whiteBoard = accuracy_test.number2image(whiteBoard, block_n, block_m, H)
                    if circle_view:
            ...
            if K == 's':
                push_watch_button = True
            else:
                push_watch_button = False
            Key_action.trigger = push_watch_button
            ...
            #print(key_action.determine_state_passive_wait())
            #print(key_action.nowa_time, key_action.state_time, key_action.nowa_time_before)

            if push_watch_button or skip_flug:
                state_num += 1
            print(state_num, step_num, (step_num - 20), (step_num - 20) < state_num < step_num)

            if (step_num - 20) < state_num < step_num:
                # center
                print('center')
            elif state_num == step_num:
                step_num += 20
                state_num += 1     
            
            elif (step_num - 20) < state_num < step_num:
                # rect 00
                print('rect 00')
                skip_flug = True
            elif state_num == step_num:
...
def whiteBoard_cace(case_style, case_num, wB_size_w, wB_size_h):

    if case_style == 'A':
        pass
    if case_style == 0:
        if case_num == 0:
            whiteBoard = np.full((wB_size_h, wB_size_w, 3), 255, np.uint8)
        elif case_num == 1:
            whiteBoard = accuracy_test.number2image(whiteBoard, block_n, block_m, H)
